...
PointCloudMapping::PointCloudMapping(double resolution_)
{
    ...
    viewerThread = boost::make_shared<std::thread>( bind(&PointCloudMapping::viewer, this ) );
}

void PointCloudMapping::shutdown()
{
    {
        unique_lock<std::mutex> lck(shutDownMutex);
        shutDownFlag = true;
...
void PointCloudMapping::insertKeyFrame(KeyFrame* kf, cv::Mat& semantic_color,cv::Mat& semantic,cv::Mat& color, cv::Mat& depth)
{
    ...
    unique_lock<std::mutex> lck(keyframeMutex);
...
void PointCloudMapping::viewer()
{
    ...
    {
        {
            unique_lock<std::mutex> lck_shutdown( shutDownMutex );
            if (shutDownFlag)
            {
                break;
            }
        ...
        {
            unique_lock<std::mutex> lck_keyframeUpdated( keyFrameUpdateMutex );
