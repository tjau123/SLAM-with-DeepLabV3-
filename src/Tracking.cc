...
Tracking::Tracking(mySystem *pSys, ORBVocabulary* pVoc, Map *pMap, boost::shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
   mSensor(sensor), mbOnlyTracking(false), mbVO(false), mbNewSegImgFlag(false),mpORBVocabulary(pVoc), mpPointCloudMapping( pPointCloud ),mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    if(sensor==mySystem::STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);

    if(sensor==mySystem::MONOCULAR)
...
void Tracking::SetSegment(mySegment* segment)
{
    mpSegment=segment;
}
...
{
    unique_lock<std::mutex> lock(mpSegment->mMutexGetNewImg);
...
void Tracking::Track()
{
    ...
    unique_lock<std::mutex> lock(mpMap->mMutexMapUpdate);

    if(mState==NOT_INITIALIZED)
    {
        if(mSensor==mySystem::STEREO || mSensor==mySystem::RGBD)
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==mySystem::MONOCULAR)
        return;
...
bool Tracking::TrackWithMotionModel()
{
   ORBmatcher matcher(0.9,true);
    ...
    int th;
    if(mSensor!=mySystem::STEREO)
        th=15;
    else
        th=7;
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==mySystem::MONOCULAR);

    // If few matches, uses a wider window search
    if(nmatches<20)
    {
        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==mySystem::MONOCULAR);
    }
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            else if(mSensor==mySystem::STEREO)
                mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    const bool c1c =  mSensor!=mySystem::MONOCULAR && (mnMatchesInliers<nRefMatches*0.25 || ratioMap<0.3f) ;
