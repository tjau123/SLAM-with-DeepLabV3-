...
mySystem::mySystem(const string &strVocFile, const string &strSettingsFile, const string &pascal_prototxt, const string &pascal_caffemodel, const string &pascal_png, const eSensor sensor,
			   ...
			   mbDeactivateLocalizationMode(false)
{
    ...
    //mpSegment =new mySegment( pascal_prototxt, pascal_caffemodel, pascal_png);
    mpSegment =new mySegment(pascal_caffemodel, pascal_png);
    mptSegment =new std::thread(&ORB_SLAM2::mySegment::Run,mpSegment);
...
cv::Mat mySystem::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp)
{
    if(mSensor!=STEREO)
    {
...
cv::Mat mySystem::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
...
//    cv::Mat aa=mpTracker->GrabImageRGBD(im,depthmap,timestamp);
//    cv::imshow("predict",aa);
...
cv::Mat mySystem::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
...
void mySystem::ActivateLocalizationMode()
{
    unique_lock<std::mutex> lock(mMutexMode);
    mbActivateLocalizationMode = true;
}

void mySystem::DeactivateLocalizationMode()
{
    unique_lock<std::mutex> lock(mMutexMode);
...
bool mySystem::SaveMap(const string &filename)
{
    cerr << "System Saving to " << filename << endl;
    return mpMap->Save(filename);
}

int mySystem::GetStatus()
{
    return mpTracker->mState;
}

void mySystem::SetStatus(int status)
{
    mpTracker->mState = (enum Tracking::eTrackingState)status;
}

void mySystem::SaveTrajectoryTUM(const string &filename)
{
...
void mySystem::SaveKeyFrameTrajectoryTUM(const string &filename)
{
...
void mySystem::SaveTrajectoryKITTI(const string &filename)
{
    cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
