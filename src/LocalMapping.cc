...
void LocalMapping::RequestStop()
{
    ...
    unique_lock<std::mutex> lock2(mMutexNewKFs);
    mbAbortBA = true;
}
...
{
    unique_lock<std::mutex> lock(mMutexStop);
    unique_lock<std::mutex> lock2(mMutexFinish);
    if(mbFinished)
        return;
    mbStopped = false;
...
{
    unique_lock<std::mutex> lock(mMutexAccept);
...
void LocalMapping::RequestReset()
{
    ...
    {
        {
            unique_lock<std::mutex> lock2(mMutexReset);
            if(!mbResetRequested)
                break;
        }
        usleep(3000);
...
void LocalMapping::SetFinish()
{
    unique_lock<std::mutex> lock(mMutexFinish);
    mbFinished = true;    
    unique_lock<std::mutex> lock2(mMutexStop);
    mbStopped = true;
}
