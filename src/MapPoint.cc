    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
{
    ...
    unique_lock<std::mutex> lock(mpMap->mMutexPointCreation);
    mnId=nNextId++;
}
...
{
    unique_lock<std::mutex> lock2(mGlobalMutex);
    unique_lock<std::mutex> lock(mMutexPos);
    Pos.copyTo(mWorldPos);
}
...
void MapPoint::SetBadFlag()
{
    ...
    {
        unique_lock<std::mutex> lock1(mMutexFeatures);
        unique_lock<std::mutex> lock2(mMutexPos);
        mbBad=true;
        obs = mObservations;
        mObservations.clear();
    }
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    {
        unique_lock<std::mutex> lock3(mMutexPos);
        mfMaxDistance = dist*levelScaleFactor;
